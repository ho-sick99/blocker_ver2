  ...
  async load_contract() {
    ...
    try {
      ...
      switch (contractData.contract_type) {
        case "n_signed": // 미체결
          response = await N_SignedContractStorage.load_contracts_info(contractData.id); // 유저 id를 인수로 현재 계정의 미체결 계약서의 정보를 요청
          break;
        case "signing": // 진행중
          response = await SigningContractStorage.load_signing_contracts_info(contractData.id); // 유저 id를 인수로 현재 계정의 진행중 계약서의 정보를 요청
          break;
        case "signed": // 체결
          response = await SignedContractStorage.load_signed_contracts_info(contractData.id); // 유저 id를 인수로 현재 계정의 체결 계약서의 정보를 요청
          break;
        default: // 계약서 타입 오류
