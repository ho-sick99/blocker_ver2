...
class SignedContractStorage {
  // 체결 계약서들 정보 로드
  static load_signed_contracts_info() {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT contract_id, id, title, contractors FROM blocker_db.signed_contract;",
        (err, result) => {
          if (err) {
            reject(`${err}`);
          } else {
  ...
  static insert_contract(contractData) {
    return new Promise((resolve, reject) => {
      db.query(
        "INSERT INTO blocker_db.signed_contract(title, content, id, contractors) VALUES(?, ?, ?, ?);", // 입력된 정보를 바탕으로 db에 계약서 추가
        [
          contractData.title,
